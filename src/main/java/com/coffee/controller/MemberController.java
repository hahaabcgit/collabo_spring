...
@RestController //해당클래스는 웹 요청( from react)을 접수하여 처리해주는 컨트롤러 클래스입니다.
...
public class MemberController {
    ...
    @PostMapping("/signup")
    public ResponseEntity<?> signup(@Valid @RequestBody Member bean, BindResult bindingResult){ //회원 가입을 위한 컨트롤러 메소드
        ...
        if(bindingResult.hasErrors()){ //유효성 검사에 문제가 있음.
            //Map<칼럼이름, 오류메시지> 형식으로 만들어서 클라이언트에게 전송해 줍니다.
            Map<String, String> errors = new HashMap<>();

            for(FieldError err:bindingResult.getFeldErrors){
